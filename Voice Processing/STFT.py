...
for folder in tqdm(folder_name):
    for i in range(no_of_records):
        ...
        stft_ = librosa.core.stft(wav_file, n_fft=512, hop_length=int(np.ceil(0.01 * sr)),
                                  win_length=int(np.ceil(0.025 * sr)), window='hann', center=True, pad_mode='reflect')
